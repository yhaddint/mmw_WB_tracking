function [ capacity_prop, capacity_true, delay_err, AOA_err ] = capacity_eval_v3(noise_pow, t_range, speed_rotate, plot_para, MCindex, L)
...
%     L = 1;
    ...
    for kk=1:Nfft
        ...
        % svd on true channel
        H_error = (randn(Nr,Nt)+1j*randn(Nr,Nt))/sqrt(2)*H_error_scaler*2;
        [U,S,V] = svd(H_k + H_error);
        [diag_S, temp_idx] = sort(diag(S), 'descend');
        S_sorted  = diag(diag_S);
        U_sorted = U(:,temp_idx);
        V_sorted = V(:,temp_idx); 
        ...
        gain_prop = U_est{kk}' * H_k *V_est{kk};
        gain_true = U_true{kk}' * H_k *V_true{kk};

        
        if L==1
            SINR_k_prop(1, kk) = abs(gain_prop(1,1))^(2)/(noise_pow);
            
            SINR_k_true(1, kk) = abs(gain_true(1,1))^(2)/(noise_pow);

        elseif L==2
            SINR_k_prop(1, kk) = abs(gain_prop(1,1))^(2)/(abs(gain_prop(1,2)).^(2)+ noise_pow);
            SINR_k_prop(2, kk) = abs(gain_prop(2,2))^(2)/(abs(gain_prop(2,1)).^(2)+ noise_pow);
    ...
    if L==1
        capacity_prop(tt) = mean(log2(1+SINR_k_prop)); % bps/Hz
        capacity_true(tt) = mean(log2(1+SINR_k_true)); % bps/Hz
    elseif L==2
        capacity_prop(tt) = mean(sum(log2(1+SINR_k_prop))); % bps/Hz
    ...
    delay_err(:,1) = mean(raydelay_est_BB1,2) - mean(raydelay_true_BB1,2);
    delay_err(:,2) = mean(raydelay_est_BB2,2) - mean(raydelay_true_BB2,2);

    AOA_err(:,1) = mean(rayAOA_est_BB1,2) - mean(rayAOA_true_BB1,2);
    AOA_err(:,2) = mean(rayAOA_est_BB2,2) - mean(rayAOA_true_BB2,2);
