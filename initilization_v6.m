...
for gg=1:G
    Psi(:,gg) = exp(-1j*(2*pi)*krange*tau_range(gg)/(Nfft*1e-9))*exp(-1j*2*pi*tau_range(gg)*28e9);
end
    ...
    % Alternative estimation parameters
        ...
        for kk=1:Nfft
            ...
            BigAlpha(kk,:) = exp(-1j*2*pi*kk*(raydelay_prev(1,:))/(1e-9*Nfft))...
                .*exp(-1j*2*pi*(raydelay_prev(1,:))*28e9)...
...
%             bigPhi = sin(rayAOA_prev(1,:))-sin(phi_round1);
            bigTheta = -sin(rayAOD(1,:))+sin(theta1);
            BigTau(kk,:) = (-1j*2*pi/(Nfft*1e-9)*kk-1j*2*pi*28e9)...
...
%             bigPhi = sin(rayAOA_prev(1,:))-sin(phi_round1);
            ...
            for ray_index=1:ray_num
                ...
                .*exp(-1j*2*pi*(tau_est(ray_index))*28e9)...
...
rho = 1;
speed_rotate = -500/180*pi;
t_range = (0.05:0.05:2)*1e-3;
...
%     L = cluster_num;
    ...
    H_freq_est = get_H_freq3([alpha_est_BB1; alpha_est_BB2],...
