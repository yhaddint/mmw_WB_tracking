function [ capacity_prop, capacity_true, delay_err, AOA_err ] = capacity_eval(noise_pow, t_range, speed_rotate, plot_para, MCindex)
...
for tt = 1:length(t_range)
    ...
    for kk=1:Nfft
        H_BB1(kk,tt) = arx1'*squeeze(H_freq_noisy(:,:,kk))*atx1;   
        H_BB2(kk,tt) = arx2'*squeeze(H_freq_noisy(:,:,kk))*atx2;   
    end
...
%         raygain_est_BB2(tt,:) = raygain(2,:)/norm_factor;
    ...
    for kk=1:Nfft
        ...
        for ll=1:L
            SINR_k_prop(ll, kk) = abs(gain_prop(ll,ll))^(2)...
                        / (sum(abs(gain_prop(ll,:)).^2) -abs(gain_prop(ll,ll))^2 + noise_pow);
            SINR_k_true(ll, kk) = abs(gain_true(ll,ll))^(2)...
                        / (sum(abs(gain_true(ll,:)).^2) -abs(gain_true(ll,ll))^2 + noise_pow);
        end
